...
requires:
  - phase: 05-bauteil-kennwerte-auf-den-crafting-karten (Plan 01)
...
tech-stack:
  ...
  patterns:
    ...
    - "Gatter-Spiegelung aus Plan 05-01 fortgeschrieben: toneFromWeaponCategoryPath() ist in src/lib/crafting.ts und scripts/verify-crafting-specs.mjs Zeile fuer Zeile identisch"
...
key-decisions:
  ...
  - "Referenzkarten in tests/e2e/crafting-specs.test.js ERGAENZT statt ersetzt — die 5 Karten aus Plan 05-01 (Allegro/Atlas/Hemera/Erebos/Drift) bleiben Teil der Liste, die 6 neuen (Lotus/Cassandra/Cirrus/AD4B Ballistic Gatling/9-Series Longsword Cannon/Agure) kommen dazu"
  ...
  - id: D4
    ...
    verification:
      - kind: e2e
        ...
        status: pass
      - kind: unit
        ref: "npm run verify:crafting — Pruefblock 3 (unveraendert aus Plan 05-01, weiterhin 5/10/0 Abweichungen)"
        status: pass
    human_judgment: false
  - id: D5
    ...
    verification:
      - kind: other
        ref: "node -e mit fs.statSync + zlib.gzipSync gegen dist/topics/crafting.html und dist/de/topics/crafting.html nach dem Build; Deltas gegen die 05-01-Grundlinie in dieser SUMMARY"
        status: pass
    human_judgment: false
...
# Phase 5 Plan 2: Vollausrollung der Bauteil-Kennwerte Summary
...
- **Tasks:** 2/2
- **Files modified:** 4 (alle bereits aus Plan 05-01 vorhanden, keine Neuanlage)
...
- `scripts/verify-crafting-specs.mjs` um 3 Pruefbloecke erweitert (7: Schiffswaffen-Ton,
  ...
  05-01 bleiben unveraendert bestehen und liefern weiterhin dieselben Zahlen
...
- Kollisionspruefung (Pruefblock 3, unveraendert aus 05-01): weiterhin **5 gesperrte Gruppen, 10 Karten, 0 Abweichungen**
...
| Datei | Grundlinie (05-01, roh) | Nachher (roh) | Delta roh | Grundlinie (gzip) | Nachher (gzip) | Delta gzip |
|---|---|---|---|---|---|---|
...
- **Referenzkarten ergaenzt, nicht ersetzt:** die 5 Karten aus Plan 05-01 bleiben im
  Test, die 6 neuen Referenzkarten (inkl. der beiden Schiffswaffen-Belege und Agure
  als S0-Beleg) kommen dazu — der Test deckt damit weiterhin den in 05-01 bewiesenen
...
- Plan 05-03 (laut `affects`) kann auf `blueprintSpecs()` als stabile, vollstaendige
  Quelle aufbauen — Groesse/Grade als Filter (Claude's Discretion aus 05-CONTEXT.md)
