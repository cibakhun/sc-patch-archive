...
requires:
  - phase: 05-bauteil-kennwerte-auf-den-crafting-karten (Plan 01+02)
...
key-decisions:
  - "Ton bekommt bewusst KEINE eigene Filtergruppe — nur die Freitextsuche deckt d.tone ab (Scope Fence aus 05-CONTEXT.md nennt ausdruecklich nur Groesse und Grade als Filter)"
...
coverage:
  - id: D1
    ...
    verification:
      - kind: e2e
        ref: "tests/e2e/crafting-specs.test.js#Filter Groesse und Grade in der Seitenleiste (05-03, DE+EN) — acht/vier Ankreuzfelder mit den Werten 0-7/A-D"
        status: pass
      - kind: other
  ...
  - id: D5
    ...
    verification:
      - kind: other
        ref: "node -e mit fs.statSync + zlib.gzipSync gegen dist/topics/crafting.html und dist/de/topics/crafting.html nach dem Build; Delta gegen die 05-02-Grundlinie in dieser SUMMARY"
        status: pass
    human_judgment: false
  - id: D6
...
# Phase 5 Plan 3: Filter nach Groesse und Grade Summary
...
freigegeben, keine Abweichung gefunden. Phase 5 ist damit abgeschlossen (3/3 Plaene).**
...
- `assets/crafting-app.js`: die Chip-Reihe `ul.cbp__spec` wird im **richtigen** Codepfad
  ...
  nicht kennt. Zuordnung nach dem Markup-Vertrag aus Plan 05-01: `li.tone` → `d.tone`
...
- `tests/e2e/crafting-specs.test.js` — neue `describe`-Gruppe „Filter Groesse und Grade in der
  Seitenleiste (05-03, DE+EN)" mit 6 Subtests
...
| `npm run audit:site` Exit-Code | **0** (0 FEHLER, 4 vorbestehende A11y-WARNUNGEN, unveraendert gegenueber 05-02) |
...
- **Kein Ton-Filter**, nur Freitextsuche: der Scope Fence aus `05-CONTEXT.md` nennt ausdruecklich
...
Anzeige-Einschraenkung ohne Serverbezug (siehe `<threat_model>` des Plans, T-05-08: „accept").
Die Chip-Werte werden nur zugeordnet und verglichen, nichts wird als HTML zurueckgeschrieben
(T-05-07: „mitigate", durch Astro-Escaping bereits erfuellt).
