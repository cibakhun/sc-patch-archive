...
**57/57, null Abweichungen** — allerdings ausschließlich für Quantumdrives, denn nur
die tragen eine unabhängige Größe im Kategorie-String. Der Namens-Join ist damit für
diesen Ausschnitt gedeckt, **nicht für den Rest** (siehe Nachtrag unten).

⚠ **Die guid-Route steht gar nicht zur Wahl:** `assets/crafting-db.json` führt kein
`entity_guid`. Die Blueprint-Felder sind abschließend `name`, `category`,
`craft_time_seconds`, `tiers`, `item_stats`, `ingredients`, `missions`. Die guid gäbe
es erst nach einer Skriptänderung plus Data.p4k-Lauf.
...
## Nachtrag 06.08. — die Dubletten sind zu 1/3 echte Kollisionen

Der Plan-Prüfer hat die 15 Namensgruppen nachgemessen, statt sie nur zu zählen.
Ergebnis, von mir gegengeprüft: **5 der 15 Gruppen sind nachweislich verschiedene
Items** — gleicher Name, gleiche Kategorie, aber abweichende `item_stats`:
...
Masse skaliert bei Bauteilen mit der Größe — Faktor 8 bei `main powerplant` ist keine
Messtoleranz, sondern ein anderes Bauteil. Weil `itemForBlueprint()` per Name joint,
träfen beide Karten denselben Eintrag; mindestens eine zeigte fremde Kennwerte. Exakt
der Fall aus [[display-name-not-a-key]].

**Das rettet die 57/57-Messung nicht** — sie deckte nur Quantumdrives ab, und in dieser
Kategorie liegt keine der fünf Kollisionen. Die Messung belegt also weniger, als sie
auf den ersten Blick scheint.

Konsequenz: Kollidierende Gruppen bleiben chiplos, und die Sperrliste wird aus den
Daten abgeleitet (abweichende `item_stats` innerhalb einer Namensgruppe), nicht von
Hand gepflegt. Die verbleibenden 10 Gruppen sind Lackvarianten mit identischen Werten
und unkritisch.
