...
provides:
  ...
  - "gemessene Seitengewicht-Grundlinie (roh+gzip, DE+EN) fuer Plan 08-02"
affects: [08-02-ausrollen-auf-alle-karten, 08-03]
...
coverage:
  ...
  - id: D3
    ...
    human_judgment: false
  - id: D4
    description: "Seitengewicht-Grundlinie (roh+gzip, DE+EN) gemessen und dokumentiert fuer Plan 08-02"
    requirement: CRAFT-01
    verification:
      - kind: other
...
# Phase 8 Plan 1: Quantumdrive-Kennwerte (Tracer) Summary
...
- `ul.cbp__spec`-Chip-Reihe auf den Quantumdrive-Karten in `CraftingApp.astro` (ein Koerper fuer DE+EN), hinter der Ausrollsperre `SPEC_ROLLOUT` (nur diese eine Kategorie, entfernt in Plan 08-02)
...
Rechnerisch ~147 Bytes roh / ~18 Bytes gzip je Chip-Reihe (57 Reihen). Plan 08-02 skaliert
...
- Die Chip-Kette (Daten -> `blueprintSpecs()` -> `sp`-Feld -> Markup -> CSS -> gebautes HTML)
  ...
  fehlschlagen kann.
- Plan 08-02 kann `SPEC_ROLLOUT` entfernen und den Kategorie-Filter in `cardsData` auf alle
...
- Grundlinie und Delta-Faktor (oben) liegen bereit, damit Plan 08-02 gegen ein Seitengewichts-
  Budget rechnen kann.
- `COLLIDING_NAMES` und `verify:crafting` decken bereits alle 15 Namensgruppen ab — Plan 08-02
