...
provides:
  - "scripts/verify-shipconsole.mjs — acht Zusicherungen gegen den ZIELZUSTAND der Konsole (Rail<->System-Bijektion, P-3-Markerzahl, D-02-Sichtbarkeit ohne JS, Textbestands-Klinke, Sprachparitaet), vorgefuehrt rot, ausgesetzt bis 16-05-PLAN.md"
...
# Phase 16 Plan 2: Konsolen-Tor -- vorgefuehrt rot, ausgesetzt bis Welle 5 Summary
...
- `.planning/phases/16-das-schiff-ist-die-navigation/deferred-items.md` - Wellen-Einschaetzung fuer `#holoact` korrigiert
...
- **`#holoact` (deferred-items.md) wurde NICHT in dieser Welle behoben**, wie im Auftrag ausdruecklich erlaubt ("wenn nicht geplant, explizit sagen"): 16-02-PLAN.md's Tasks beruehren ausschliesslich `scripts/verify-shipconsole.mjs`, `scripts/lib/gate-registry.mjs`, `package.json`, `scripts/verify-shipcard.mjs` — kein Markup, kein CSS. Der Knopf ist weiterhin tot und wird laut `16-UI-SPEC.md` Detailvertrag Punkt 7 (Ladeausloeser Klick->Scroll, D-04) in einer spaeteren Welle (voraussichtlich 15-03/15-04) ersatzlos entfernt. `deferred-items.md`s Wellen-Einschaetzung war eine Vermutung des Vorgaenger-Plans, keine Zusage von 15-02 — korrigiert statt stillschweigend falsch stehen gelassen.
...
- **Scharfschaltung ist Aufgabe von `16-05-PLAN.md`**: Registry-Eintrag `disabled` entfernen, `verify:shipcard`s eigene Rot-Vorfuehrung der Konsolen-Erweiterung nachholen (Bewaffnungswert absichtlich doppelt stehen lassen, siehe `16-UI-SPEC.md` Punkt 11.5).
