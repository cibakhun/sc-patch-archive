...
| `schiffskonsole-messung.mjs` | Belegen die drei bindenden Punkte P-1/P-2/P-3 aus `15-UI-SPEC.md` § 3a (Phase 15, „Das Schiff ist die Navigation") am ARTEFAKT statt aus der Konfiguration: `--census` (kein Browser) zaehlt am ausgelieferten `#holodata` in `dist/`, rechnet drei P-3-Varianten aus der Quelle nach und prueft sie gegen; ohne `--census` misst sie am gerenderten Bildpunkt (Fuellgrad, Markergroesse, Dauerlabels, Buehnenbreite) gegen einen laufenden Vorschau-Server. Braucht `playwright-core` + einen installierten Chrome/Edge fuer den Browser-Teil. | [`15-01-SUMMARY.md`](../../.planning/phases/15-das-schiff-ist-die-navigation/15-01-SUMMARY.md) |
